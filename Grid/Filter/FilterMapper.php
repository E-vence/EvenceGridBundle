...
class FilterMapper {
     ...
     * Class constructor
     * 
     * @param string $id Doctrine ORM column identifier 
     * @param string $field FormInterface name
     * @param string $type Type of the expression
     * @param unknown $args Array of arguments
     */
     ...
     * @param string $id Doctrine ORM column identifier 
     * @param string $field FormInterface name
     * @param bool $strict Whether or not to use strict like or just include wildcards
     * @return \Evence\Bundle\GridBundle\Grid\Filter\FilterMapper
     */
    static function like($id, $field, $strict = false){
        return new FilterMapper($id, $field, self::FILTER_TYPE_LIKE, ['strict' => $strict]);
    }
    ...
    static function notLike($id, $field, $strict = false){
        return new FilterMapper($id, $field, self::FILTER_TYPE_NOTLIKE, ['strict' => $strict]);
    }
    /**
    ...
    static function between($id, $field, $secondField){
        return new FilterMapper($id, $field, self::FILTER_TYPE_BETWEEN, ['secondField' => $secondField]);
    }
    ...
    public function filterQuery(QueryBuilder $qb, FormInterface $form, $prefix = 'e'){
        ...
        switch($this->type){
            case self::FILTER_TYPE_EQ:
                $qb->andWhere($qb->expr()->eq($id, ':'.$param))->setParameter($param, $data);
                break;
            case self::FILTER_TYPE_LT:
                $qb->andWhere($qb->expr()->lt($id, ':'.$param))->setParameter($param, $data);
                break;
    ...
    public function getData(FormInterface $form, $field = null){
        if(!$field) return $form->get($this->field)->getData();
        else return $form->get($field)->getData();
    }
